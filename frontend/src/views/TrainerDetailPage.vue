<template>
  <div class="max-w-3xl mx-auto px-4 py-6 sm:py-10">
    <router-link
      ...
      class="inline-flex items-center gap-2 text-slate-400 hover:text-white mb-6 sm:mb-8 transition font-medium text-sm sm:text-base"
    >
    ...
    <div v-if="loading" class="space-y-6">
      <div class="h-28 w-28 rounded-full bg-white/10 animate-pulse mx-auto" />
      <div class="h-6 bg-white/10 rounded w-40 mx-auto animate-pulse" />
      <div class="h-4 bg-white/5 rounded w-56 mx-auto animate-pulse" />
    </div>
    ...
    <template v-else-if="trainer">
      <!-- 프로필 카드 -->
      <section class="rounded-2xl border border-white/10 bg-[#141414] overflow-hidden mb-6 sm:mb-8">
        <div class="p-6 sm:p-8 flex flex-col sm:flex-row items-center sm:items-start gap-6">
          <div
            v-if="trainer.profileImage || trainer.image"
            class="w-28 h-28 sm:w-32 sm:h-32 rounded-full bg-cover bg-center ring-4 ring-white/10 shadow-md shrink-0"
          ...
          <div class="flex-1 text-center sm:text-left min-w-0">
            <h1 class="text-2xl sm:text-3xl font-bold text-white">{{ trainer.name }}</h1>
            <p class="text-primary-400 font-semibold mt-1">{{ trainer.specialty }}</p>
            <p class="text-slate-400 text-sm mt-1">{{ trainer.experience }}</p>
            <div class="flex flex-wrap items-center justify-center sm:justify-start gap-3 mt-3">
              <StarRating :value="trainer.rating ?? 0" />
              <span v-if="trainer.reviewCount != null" class="text-slate-400 text-sm">
                리뷰 {{ formatReviewCount(trainer.reviewCount) }}개
              </span>
            </div>
            <p class="mt-4 text-lg font-bold text-white">
              세션당 {{ formatPrice(trainer.pricePerHour ?? 0) }}
            </p>
            <router-link
              to="/reserve"
      ...
      <!-- 트레이너 소개 -->
      <section class="rounded-2xl border border-white/10 bg-[#141414] p-6 sm:p-8 mb-6 sm:mb-8">
        <h2 class="text-lg font-bold text-white mb-4">트레이너 소개</h2>
      ...
      <!-- 전문 트레이닝 -->
      <section v-if="trainer.trainingAreas?.length" class="rounded-2xl border border-white/10 bg-[#141414] p-6 sm:p-8 mb-6 sm:mb-8">
        <h2 class="text-lg font-bold text-white mb-4">전문 트레이닝</h2>
        <ul class="space-y-3">
          <li
          ...
          >
            <span class="flex-shrink-0 w-8 h-8 rounded-lg bg-primary-900/50 text-primary-400 flex items-center justify-center">
      ...
      <!-- 자격 및 경력 -->
      <section v-if="trainer.certifications?.length" class="rounded-2xl border border-white/10 bg-[#141414] p-6 sm:p-8 mb-6 sm:mb-8">
        <h2 class="text-lg font-bold text-white mb-4">자격 및 경력</h2>
        <ul class="space-y-3">
          <li
          ...
          >
            <span class="flex-shrink-0 w-8 h-8 rounded-lg bg-white/10 text-slate-400 flex items-center justify-center mt-0.5">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <!-- PT 프로그램 -->
      <section v-if="trainer.programs?.length" class="rounded-2xl border border-white/10 bg-[#141414] p-6 sm:p-8">
        <h2 class="text-lg font-bold text-white mb-4">PT 프로그램</h2>
        <ul class="space-y-3">
          <li
            ...
            class="flex items-center gap-3 text-slate-300"
          >
            <span class="flex-shrink-0 w-8 h-8 rounded-lg bg-emerald-900/50 text-emerald-400 flex items-center justify-center">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
