  ...
  <div class="max-w-4xl mx-auto px-4 py-6 sm:py-8">
    <header class="mb-6 sm:mb-8 flex items-center justify-between gap-4 flex-wrap">
      <div>
        ...
        <p class="mt-1 text-slate-400 text-sm">고객이 남긴 무료 상담 문의를 확인합니다.</p>
      </div>
      <!-- 필터 -->
      <div class="flex gap-2">
        <button
          ...
          :class="filter === opt.value
            ...
            : 'bg-[#141414] border border-white/10 text-slate-300 hover:bg-white/5'"
    ...
    <template v-else>
      <div v-if="filtered.length === 0" class="text-center text-slate-400 py-16 bg-[#141414] rounded-xl border border-white/10">
      ...
      <div v-else class="space-y-3">
        <div
          ...
          :key="item.inquiryId"
          class="bg-[#141414] rounded-xl border transition cursor-pointer"
        ...
        >
          <div class="px-5 py-4 flex items-start gap-4">
            ...
            <span
              ...
              :class="item.status === 'UNREAD' ? 'bg-amber-950/30 text-amber-400' : 'bg-white/5 text-slate-400'"
            >
            ...
            <div class="flex-1 min-w-0">
              ...
              <p v-if="item.phone" class="text-xs text-slate-400 mt-0.5">{{ item.phone }}</p>
              <p class="text-sm text-slate-300 mt-1.5 line-clamp-2">{{ item.message }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <Transition name="panel-slide">
      <div v-if="selected" class="fixed inset-0 z-50 flex justify-end" @click.self="closePanel">
        ...
        <div class="relative w-full max-w-lg bg-[#141414] shadow-2xl flex flex-col h-full overflow-y-auto">
          <div class="flex items-center justify-between px-6 py-5 border-b border-white/10 sticky top-0 bg-[#141414] z-10">
            <h2 class="font-bold text-white text-lg">문의 상세</h2>
            <button type="button" class="text-slate-400 hover:text-slate-300 text-2xl leading-none" @click="closePanel">×</button>
          </div>
          ...
          <div class="px-6 py-5 flex-1 space-y-5">
            ...
            <dl class="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <dd class="font-semibold text-white">{{ selected.name }}</dd>
              </div>
              <div>
                <dt class="text-slate-400 text-xs mb-0.5">연락처</dt>
                <dd class="font-semibold text-white">{{ selected.phone || '—' }}</dd>
              </div>
              <div class="col-span-2">
                <dt class="text-slate-400 text-xs mb-0.5">접수일시</dt>
                <dd class="text-slate-300">{{ formatDateFull(selected.createdAt) }}</dd>
              </div>
            </dl>
            ...
            <div>
              ...
              <div class="bg-white/5 rounded-xl p-4 text-sm text-slate-300 leading-relaxed whitespace-pre-wrap">{{ selected.message }}</div>
            </div>

            <!-- 상태 -->
            <div class="flex items-center gap-3">
              <span class="text-sm text-slate-400">상태:</span>
              <span
                class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold"
                :class="selected.status === 'UNREAD' ? 'bg-amber-950/30 text-amber-400' : 'bg-emerald-950/30 text-emerald-400'"
              >
          ...
          <div class="px-6 py-4 border-t border-white/10 flex gap-3">
            ...
            <button
              ...
              type="button"
              :disabled="actionLoading"
              class="flex-1 py-2.5 rounded-xl font-semibold bg-white/5 text-slate-300 hover:bg-white/10 disabled:opacity-50 transition text-sm"
              @click="markAs('UNREAD')"
            >
            ...
            <a
              ...
              class="px-4 py-2.5 rounded-xl font-semibold border border-white/10 text-slate-300 hover:bg-white/5 transition text-sm flex items-center gap-1.5"
            >
