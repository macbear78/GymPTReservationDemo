  ...
  <div class="min-h-screen bg-slate-50 flex items-center justify-center px-4 py-12">
    <div class="w-full max-w-md">
      ...
      <div class="text-center mb-8">
        <router-link to="/" class="inline-flex items-center gap-2 text-slate-700 hover:text-slate-900 transition">
      ...
      <div class="flex rounded-xl border border-slate-200 bg-white p-1 mb-6 shadow-sm">
        <button
          ...
          :class="tab === 'login' ? 'bg-primary-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'"
          @click="tab = 'login'"
        >로그인</button>
        <button
          type="button"
          class="flex-1 py-2.5 rounded-lg text-sm font-semibold transition"
          :class="tab === 'register' ? 'bg-primary-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'"
          @click="tab = 'register'"
        >회원가입</button>
      </div>
      ...
      <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
        ...
        <div v-if="tab === 'register'">
          <label class="block text-sm font-medium text-slate-700 mb-1.5">이름 <span class="text-red-400">*</span></label>
          <input
            v-model="form.name"
            type="text"
            placeholder="홍길동"
            maxlength="20"
            class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:ring-2 focus:ring-primary-500 outline-none text-sm"
          />
        </div>

        <!-- 전화번호 -->
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1.5">전화번호 <span class="text-red-400">*</span></label>
          <input
            :value="form.phone"
            type="tel"
            inputmode="numeric"
        ...
        <!-- 비밀번호 -->
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1.5">비밀번호 <span class="text-red-400">*</span></label>
          <div class="relative">
            <input
        ...
        <!-- 비밀번호 확인 (회원가입) -->
        <div v-if="tab === 'register'">
          <label class="block text-sm font-medium text-slate-700 mb-1.5">비밀번호 확인 <span class="text-red-400">*</span></label>
          <input
            v-model="form.confirm"
            :type="showPw ? 'text' : 'password'"
            placeholder="비밀번호 재입력"
            class="w-full px-4 py-2.5 rounded-xl border focus:ring-2 focus:ring-primary-500 outline-none text-sm"
            :class="form.confirm && form.confirm !== form.password ? 'border-red-300' : 'border-slate-300'"
            @keyup.enter="submit"
          />
          <p v-if="form.confirm && form.confirm !== form.password" class="mt-1 text-xs text-red-500">비밀번호가 일치하지 않습니다.</p>
        </div>

        <p v-if="errorMsg" class="text-red-500 text-sm bg-red-50 rounded-lg px-3 py-2">{{ errorMsg }}</p>
